package com.msa4mokhwagram.domain.auth.requests;
...
public record RegistrationReq(
    ...
    @Schema(description = "비밀번호", example = "test4@test.com", nullable = false, requiredMode = Schema.RequiredMode.REQUIRED)
    ...
    @Schema(description = "닉네임", example = "takHo", nullable = false, requiredMode = Schema.RequiredMode.REQUIRED)
    @NotBlank(message = "닉네임은 필수 항목입니다.")
    @Pattern(regexp = "^[0-9a-zA-Z_]{2,20}$", message = "허용하지 않는 닉네임 양식입니다.")
    String nick,

    @Schema(description = "프로필", example = "test4@test.com", nullable = false, requiredMode = Schema.RequiredMode.REQUIRED)
    @NotBlank(message = "프로필은 필수 항목입니다.")
    String profile
) {
    @Schema(hidden = true)
    @AssertTrue(message = "비밀번호와 비밀번호 확인이 일치하지않습니다.")
