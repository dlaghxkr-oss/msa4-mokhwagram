package com.msa4mokhwagram.domain.user.entities;
...
@Entity // 해당 클래스가 JPA 엔티티임을 선언하는 어노테이션
@EntityListeners(AuditingEntityListener.class) // 엔티티의 이벤트 리스너 지정
@Table(name = "users") // 테이블명 맵핑해주는 어노테이션
...
public class User {
    ...
    @Enumerated(value = EnumType.STRING) // Enum을 어떤 데이터 형식으로 저장할지 설정하는 어노테이션
